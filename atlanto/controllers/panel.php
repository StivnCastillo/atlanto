...
class Panel extends CI_Controller {
	...
	public function escritorio()
	{
		...
		if ($this->session->userdata('roles')->id == 1) {			
			...
			$n_mis_tickets = $this->ticket_model->get_no_tickets(array('id_usuario_asignado' => $this->session->userdata('id')));

			$data['tareas'] = $tareas;
			$data['n_mis_tareas'] = $n_mis_tareas;
			$data['n_tareas_no_terminadas'] = $n_tareas_no_terminadas;
			$data['n_tickets_no_resueltos'] = $n_tickets_no_resueltos;
			$data['n_tickets_espera'] = $n_tickets_espera;
			$data['n_mis_tickets'] = $n_mis_tickets;
		}else{
			$n_en_espera = $this->ticket_model->get_no_tickets(array('id_estado' => 5, 'id_usuario' => $this->session->userdata('id')));
			$n_tickets_no_resueltos = $this->ticket_model->get_no_tickets(array('id_estado' => 3, 'id_usuario' => $this->session->userdata('id')));
			$n_mis_tickets = $this->ticket_model->get_no_tickets(array('id_usuario' => $this->session->userdata('id')));
			$data['n_mis_tickets'] = $n_mis_tickets;
			$data['n_tickets_no_resueltos'] = $n_tickets_no_resueltos;
			$data['n_en_espera'] = $n_en_espera;
		}
