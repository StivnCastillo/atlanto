...
?>
		<article class="well">
			<form class="form-inline">
				<a href="<?php echo base_url().'componente/nuevo_tvideo/' ?>" id="btn_agregar" class="btn btn-inverse" title="<?php echo $ci->lang->line('lnk_agregar') ?>"><i class="icon icon-plus icon-white"></i></a>
			...
			<table class="table table-striped table-hover table-bordered tabla" id="tabla">
				<thead>
					<tr>
						...
						<th><?php echo $ci->lang->line('lbl_fabricante'); ?></th>
				...
				<tbody>
					...
					<?php if ($tvideo): ?>
						<?php foreach ($tvideo as $row): ?>
							<tr>
								<td><?php echo $i; ?> </td>
								<td><a href="<?php echo base_url().'componente/nuevo_tvideo/'.$row->id ?>"><?php echo $row->nombre; ?></a></td>
								<td><?php echo $row->fabricante; ?></td>
								<td><?php echo $row->memoria.' GB'; ?></td>
								<td class="tabla-center">
									<div class="btn-group">
										<a class="btn btn-small" href="<?php echo base_url().'componente/nuevo_tvideo/'.$row->id ?>">
										...
										<div id="vnt_eliminar<?php echo $row->id; ?>" class="modal hide fade" >
											<div class="modal-header">
												...
												<h3 id="myModalLabel"><?php echo $ci->lang->line('titulo_eliminar_tvideo'); ?></h3>
											</div>
											<div class="modal-body">
												<p class="lead"><?php echo $ci->lang->line('msj_eliminar_tvi'); ?> <?php echo $row->nombre; ?>?</p>
											</div>
											<div class="modal-footer">
												<button class="btn" data-dismiss="modal" aria-hidden="true"><?php echo $ci->lang->line('btn_cerrar'); ?></button>
												<a href="<?php echo base_url().'componente/eliminar_tvideo/'.$row->id ?>" class="btn btn-primary"><?php echo $ci->lang->line('btn_eliminar'); ?></a>
											</div>
										</div>
