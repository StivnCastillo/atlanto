...
?>
		<article class="well">
			<form class="form-inline">
				<a href="<?php echo base_url().'sistema_operativo/nuevo' ?>" id="btn_agregar" class="btn btn-inverse" title="<?php echo $ci->lang->line('lnk_agregar') ?>"><i class="icon icon-plus icon-white"></i></a>
			...
			<table class="table table-striped table-hover tabla" id="tabla">
				<thead>
					<tr>
						...
						<th><?php echo $ci->lang->line('tab_version'); ?></th>
				...
				<tbody>
					...
					<?php if ($so): ?>
						<?php foreach ($so as $row): ?>
							<tr>
								<td><?php echo $i; ?> </td>
								<td><a href="<?php echo base_url().'sistema_operativo/nuevo/'.$row->id ?>"><?php echo $row->nombre; ?></a></td>
								<td><?php echo $row->version; ?></td>
								<td><?php echo $row->tipo_so; ?></td>
								<td><?php echo $row->descripcion; ?></td>
								<td class="tabla-center">
									<div class="btn-group">
										<a class="btn btn-small" href="<?php echo base_url().'sistema_operativo/nuevo/'.$row->id ?>">
										...
										<div id="vnt_eliminar<?php echo $row->id; ?>" class="modal hide fade" >
											<div class="modal-header">
												...
												<h3 id="myModalLabel"><?php echo $ci->lang->line('titulo_eliminar_so'); ?></h3>
											</div>
											<div class="modal-body">
												<p class="lead"><?php echo $ci->lang->line('msj_eliminar_so'); ?> <?php echo $row->nombre; ?>?</p>
											</div>
											<div class="modal-footer">
												<button class="btn" data-dismiss="modal" aria-hidden="true"><?php echo $ci->lang->line('btn_cerrar'); ?></button>
												<a href="<?php echo base_url().'sistema_operativo/eliminar/'.$row->id ?>" class="btn btn-primary"><?php echo $ci->lang->line('btn_eliminar'); ?></a>
											</div>
										</div>
