...
?>
		<article class="well">
			<form class="form-inline">
				<a href="<?php echo base_url().'red/nuevo' ?>" id="btn_agregar" class="btn btn-inverse" title="<?php echo $ci->lang->line('lnk_agregar') ?>"><i class="icon icon-plus icon-white"></i></a>
				...
				<tbody>
					...
					<?php if ($redes): ?>
						<?php foreach ($redes as $row): ?>
							<tr>
								<td><?php echo $i; ?> </td>
								<td><a href="<?php echo base_url().'red/nuevo/'.$row->id ?>"><?php echo $row->nombre; ?></a></td>
								<td><?php echo $row->descripcion; ?></td><td class="tabla-center">
									<div class="btn-group">
										<a class="btn btn-small" href="<?php echo base_url().'red/nuevo/'.$row->id ?>">
										...
										<div id="vnt_eliminar<?php echo $row->id; ?>" class="modal hide fade" >
											<div class="modal-header">
												...
												<h3 id="myModalLabel"><?php echo $ci->lang->line('titulo_eliminar_red'); ?></h3>
											</div>
											<div class="modal-body">
												<p class="lead"><?php echo $ci->lang->line('msj_eliminar_red'); ?> <?php echo $row->nombre; ?>?</p>
											</div>
											<div class="modal-footer">
												<button class="btn" data-dismiss="modal" aria-hidden="true"><?php echo $ci->lang->line('btn_cerrar'); ?></button>
												<a href="<?php echo base_url().'red/eliminar/'.$row->id ?>" class="btn btn-primary"><?php echo $ci->lang->line('btn_eliminar'); ?></a>
											</div>
										</div>
